...
def choose_capacity(
    dataset: str, nmaquinas: int = 2, timelimit: int = 50, get_closest: bool = True
) -> Dict[str, any]:
...
def running_all_instance_choose_capacity() -> pd.DataFrame:
    ...
    df_results_optimized.to_excel(Path.resolve(Path.cwd() / "resultados" / "capacidadesfç0.xlsx"), index=False)
...
def running_all_instance_with_chosen_capacity():
    ...
    pdf_capacidades = pd.read_excel(Path.resolve(Path.cwd() / "resultados" / "capacidadesfç0.xlsx"), engine="openpyxl")
        ...
        for i in range(1, 71):
            for nmaq in [2, 4, 8]:
                ...
                best_result = solve_optimized_model(
                    dataset, capacity=cap[0], nmaquinas=nmaq, timelimit=3600
                )
    ...
    df_results_optimized = pd.DataFrame(final_results)
    df_results_optimized.to_excel(Path.resolve(Path.cwd() / "resultados" / "otimizadosfç0.xlsx"), index=False)
    pass
